...
struct PremiumFeatureTests {
    ...
    @Test func premiumFeaturesHaveDescriptions() async throws {
        for feature in PremiumFeature.allCases {
            #expect(!feature.description.isEmpty)
        }
    }
    
    @Test func premiumFeaturesHaveIcons() async throws {
        for feature in PremiumFeature.allCases {
...
struct SubscriptionStatusTests {
    ...
    @Test func subscriptionStatusDisplayText() async throws {
        #expect(!SubscriptionStatus.unknown.displayText.isEmpty)
        #expect(!SubscriptionStatus.notSubscribed.displayText.isEmpty)
        #expect(!SubscriptionStatus.subscribed(expirationDate: Date(), willRenew: true).displayText.isEmpty)
        #expect(!SubscriptionStatus.expired.displayText.isEmpty)
        #expect(!SubscriptionStatus.inGracePeriod(expirationDate: Date()).displayText.isEmpty)
    }
    
    @Test func subscriptionStatusIsActive() async throws {
        #expect(SubscriptionStatus.unknown.isActive == false)
        #expect(SubscriptionStatus.notSubscribed.isActive == false)
        #expect(SubscriptionStatus.expired.isActive == false)
        #expect(SubscriptionStatus.revoked.isActive == false)
