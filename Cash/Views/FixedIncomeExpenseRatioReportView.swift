...
struct FixedIncomeExpenseRatioReportView: View {
    ...
    @State private var fixedIncome: Decimal = 0
    ...
    private func loadData() async {
        ...
        for transaction in transactions {
            for entry in transaction.entries ?? [] {
                ...
                if account.accountClass == .income && entry.entryType == .credit {
                    income += entry.amount
                } else if account.accountClass == .expense && entry.entryType == .debit {
